...
class ShopifyStorefrontService {
  ...
  private transformProducts(shopifyProducts: any[]): AppProduct[] {
    return shopifyProducts.map((product: any) => ({
      ...
      // Use native Shopify price format - priceV2 is the correct format from Shopify API
      price: product.variants[0]?.priceV2 || product.priceRange?.minVariantPrice,
      category: product.productType || 'Art',
      variants: product.variants.map((variant: any) => ({
        id: variant.id,
        title: variant.title,
        // Use native Shopify priceV2 format directly
